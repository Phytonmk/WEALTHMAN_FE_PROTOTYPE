  ...
  app.get('/api/portfolio-history/:time/:portfolio', async (req, res, next) => {
    try {
      ...
      const callback = (response) => {
        console.log(`Generating history: ${got} / ${tokens.length}`)
        got++
        if (response) {
      ...
      for (let token of tokens) {
        ...
        console.log(`Sent history request for ${token.name}`)
        axios(`https://coinmarketcap.com/currencies/${token.name}/historical-data/?start=${startString}&end=${endString}`, {timeout}).then(callback)
         .catch(e => {
           console.log(e)
           callback(false)
         })
