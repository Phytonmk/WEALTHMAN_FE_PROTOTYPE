...
class Sortable extends Component {
  ...
  renderListing(listing, index) {
    ...
    .map((title, titleIndex) => {
      ...
      switch(title) {
        case "number":
          return (
            <div key={titleIndex} className={("number" + firstLast)}>
              {index + 1}
            </div>
          );
        case "img":
          return (
            <div key={titleIndex} className={("circle left" + firstLast)}>
        ...
        case "status":
          return (
            <div key={titleIndex} className={("status" + firstLast)}>
              <p className={listing.status}>{listing.status}</p>
            </div>
          );
        case "percent_portfolio":
          return (
            <div key={titleIndex} className={("percent-portfolio" + firstLast)}>
              {listing.percent_portfolio}
            </div>
          );
        default:
          return (
            <div key={titleIndex} className={(className + firstLast)}>
  ...
  renderTitles() {
    ...
    if (this.props.titles) {
      ...
      .map((title, index) => {
        return (
          <div key={index} className={title.class} title={title.tooltip}>
            <button
    ...
    .map((title, index) => {
      if (this.props.titles && this.props.titles[index].upper)
        if (!upperInserted)
        {
          upperInserted = true;
          return (
            <div className="upper-titles" key={index}>
      ...
      switch(title.toLowerCase()) {
        ...
        default:
          return (
            <div key={index} className={(className + firstLast)} title={this.props.titles ? this.props.titles[index].tooltip : ""}>
              <button
  ...
  render() {
    ...
    var statusSearch = (
      <select value={this.state.status} onChange={(event) => this.setState({ status: event.target.value })}>
        {["status", "cancelled", "declined", "accepted", "pending"].map((status, i) => <option key={i}><p className={status}>{status}</p></option>)}
      </select>
    );
