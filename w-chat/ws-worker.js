...
module.exports = (port) => {
  ...
  amqp.connect(configs.rabbitMq, function(err, conn) {
    if (err) {
      console.log(err)
      process.exit(1)
    } else {
    ...
    conn.createChannel(function(err, ch) {
      ...
      ch.assertQueue('', {exclusive: true}, function(err, q) {
        ...
        ch.consume(q.queue, function(msg) {
          ...
          console.log(`Got messages from exchanger on port ${port} for user ${message.companionId}, this user here: ${usersOfThisWorker[message.companionId] !== undefined}`)
