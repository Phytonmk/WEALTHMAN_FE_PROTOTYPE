...
module.exports = () => new Promise(async (resolve, reject) => {
  ...
  const requests = await Request.find({exchange_withdraw_allowed: false, status: 'buying tokens'})
  for (let request of requests) {
    ...
    if (thisRequestSentRecord === null) {
      ...
      await axios.post(`https://api.telegram.org/bot${configs.telegram.token}/sendMessage`, {
        ...
        text: `You need to accept this smart contract to be with withdrawable:\n<code>${portfolio.smart_contract}</code>\n\n/allow_${request._id}`,
        parse_mode: 'HTML'
      }).then(async () => {
